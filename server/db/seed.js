...
const created = [];
await tx(async (t) => {
  const insertRoutine = t.prepare(ROUTINE_INSERT);
  for (const [index, routine] of routines.entries()) {
    ...
    const result = await insertRoutine.run(row);
    created.push({ ...row, id: result.lastInsertRowid });
  }
});

const LOG_INSERT = `INSERT INTO logs (routine_id, user_id, log_date, status, value, completed_at)
   VALUES (?, ?, ?, ?, ?, ?)
...
let logCount = 0;
await tx(async (t) => {
  const insertLog = t.prepare(LOG_INSERT);
  for (let offset = 0; offset < DAYS; offset += 1) {
    ...
    for (const routine of created) {
      ...
      if (roll < chance) {
        ...
        await insertLog.run(routine.id, userId, date, 'done', value, `${date}T${routine.start_time || '12:00'}:00Z`);
        logCount += 1;
      } else if (routine.goal_type === 'quantity' && roll < chance + 0.14) {
        const value = Math.round(routine.target_value * (0.3 + Math.random() * 0.4));
        await insertLog.run(routine.id, userId, date, 'partial', value, `${date}T12:00:00Z`);
        logCount += 1;
      }
    }
  }
...
const JOURNAL_INSERT = `INSERT INTO journal (user_id, entry_date, mood, energy, body)
