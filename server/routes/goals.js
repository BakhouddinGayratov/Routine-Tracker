...
async function ownedGoal(userId, id) {
  const goal = (await db.prepare('SELECT * FROM goals WHERE id = ? AND user_id = ?').get(id, userId));
...
goalsRouter.get('/', asyncHandler(async (req, res) => {
  ...
  const routines = (await db.prepare('SELECT * FROM routines WHERE user_id = ? AND archived = 0').all(req.user.id));
...
goalsRouter.post('/', asyncHandler(async (req, res) => {
  ...
  const sort_order = (await db.prepare('SELECT COALESCE(MAX(sort_order), 0) + 1 AS next FROM goals WHERE user_id = ?')
  ...
  const goal = (await db.prepare('SELECT * FROM goals WHERE id = ?').get(info.lastInsertRowid));
...
goalsRouter.patch('/:id', asyncHandler(async (req, res) => {
  const existing = await ownedGoal(req.user.id, req.params.id);
  ...
  (await db.prepare(
    `UPDATE goals SET ${sets}, updated_at = utc_now()
     WHERE id = @id AND user_id = @user_id`,
  ).run({ ...data, id: existing.id, user_id: req.user.id }));

  const goal = (await db.prepare('SELECT * FROM goals WHERE id = ?').get(existing.id));
  const to = todayIn(req.user.timezone);
  const from = addDays(to, -(WINDOW_DAYS - 1));
  const routines = (await db.prepare('SELECT * FROM routines WHERE user_id = ? AND goal_id = ? AND archived = 0')
...
goalsRouter.delete('/:id', asyncHandler(async (req, res) => {
  ...
  const unlinked = (await db.prepare('SELECT COUNT(*) AS n FROM routines WHERE goal_id = ? AND user_id = ?')
    .get(goal.id, req.user.id)).n;

  (await db.prepare('DELETE FROM goals WHERE id = ? AND user_id = ?').run(goal.id, req.user.id));
